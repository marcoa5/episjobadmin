...
const url = '/api'
app.get(url+'/getusers', function(req,res){
    admin.auth().listUsers(1000).then((a)=>{
        res.send(a.users)
    })
})

app.get(url+'/getuserinfo', function(req,res){
    var id = req.query.id
...
app.get(url+'/createuser', function(req,res){
...
app.all(url+'/updateuser', function(req,res){
    var Nome = req.query.Nome
    var Cognome = req.query.Cognome
    var Pos=req.query.Pos
    var id = req.query.id
...
app.get(url+'/delete',function(req,res){
    var id = req.query.id
...
app.all(url+'/mail', function(req, res,next) {
    var arg = req.query
...
app.all(url+'/mailmod', async function(req, res,next) {
    var arg = req.query
...
app.all(url+'/maildebug', async function(req, res,next) {
    var arg = req.query
    if(arg.to1!=undefined){
        transporter.sendMail(createMailOptions(arg), (error, info)=>{
            if (error) {
...
app.get(url+'/certiq', function(req,res){
    let count = 0
    let code=''
    let machines=[]
    let lung=0
...
app.get(url+'/partreq', cors(), function(req,res){
...
app.all(url+'/psdllp',function(req,res){
    let kt=0
...
app.all(url+'/sjpdf', function(req,res){
...
app.post(url+'/sjMa', function(req,res){
    res.send(req.body)
})

app.all(url+'/sendSJNew', function(req,res){
    let g = req.body
    createMA(g)
    .then(urlMa=>{
        g.info.urlMa = urlMa
