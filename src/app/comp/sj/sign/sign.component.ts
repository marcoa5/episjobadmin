...
export class SignComponent implements OnInit {
  ...
  drawSign(img?:any,a?:number,b?:number){
    if(this.sign!='') {
      ...
      image.src=this.sign
      if(a && b) {
        ctx.drawImage(img?img:image,0,0, a,b)
      } else {
        ctx.drawImage(img?img:image,0,0)
  ...
  resizeSignaturePad(){
    let old = this.signaturePad.toDataURL()
